# PowerShell - Loop Through All VMs in a Resource Group - Enhanced Backup Policy v3
...
Write-Host "Starting Azure Backup Configuration for ALL VMs in Resource Group..." -ForegroundColor Cyan
...
Write-Host "Processing ALL VMs in resource group" -ForegroundColor Yellow
...
# Step 5: Get ALL VMs in the resource group:
Write-Host "`nStep 4: Retrieving ALL VMs from resource group..." -ForegroundColor Cyan
try {
    $VMs = Get-AzVM -ResourceGroupName $SourceResourceGroupName -ErrorAction Stop
    $accessibleVMs = @()
    
    foreach ($VM in $VMs) {
        $accessibleVMs += $VM.Name
        Write-Host "VM found: $($VM.Name)" -ForegroundColor Green
    }
    
    if ($accessibleVMs.Count -eq 0) {
        Write-Host "No VMs found in resource group: $SourceResourceGroupName" -ForegroundColor Yellow
        Write-Host "Exiting script." -ForegroundColor Yellow
        exit
    }
    
    Write-Host "Total VMs found: $($accessibleVMs.Count)" -ForegroundColor Green
}
catch {
    Write-Host "Error retrieving VMs from resource group: $($_.Exception.Message)" -ForegroundColor Red
...
# Step 8: Apply backup policy to ALL VMs:
Write-Host "`nStep 7: Applying backup policy to ALL VMs..." -ForegroundColor Cyan
...
Write-Host "Source Resource Group: $SourceResourceGroupName" -ForegroundColor White
...
Write-Host "Total VMs in resource group: $($accessibleVMs.Count)" -ForegroundColor White
Write-Host "="*50 -ForegroundColor Cyan